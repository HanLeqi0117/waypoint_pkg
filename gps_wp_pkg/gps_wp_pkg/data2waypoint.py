...
class Data2Waypoint(Node):
    ...
    def __init__(self):
        ...
        self._mode_ = self.declare_parameter("mode", "GPS", "GPS or SLAM or BOTH").get_parameter_value().string_value
        
        self._tf_buffer_ = Buffer()
        self._tf_listener_ = TransformListener(self._tf_buffer_, self)
        self._amcl_transform_ = TransformStamped()
        self._gps_transform_ = TransformStamped()
        
        self._amcl_marker_pub_ = self.create_publisher(Marker, "amcl_waypoint_marker", 100)
        self._gps_marker_pub_ = self.create_publisher(Marker, "gps_waypoint_marker", 100)
        
        self._navsat_fix_sub_ = self.create_subscription(NavSatFix, "fix", self.sub_navsat_fix, 20)
